...
begin
    ...
    try
       if (NomeBotao = 'Salvar') then
         begin
            ...
            Application.MessageBox('Execução Incluída Com Sucesso!','Atenção!',MB_OK);

         end
       else if (NomeBotao = 'Alterar') then
         begin

             if (Application.MessageBox(pchar('Deseja Alterar a Música '+cmbNomeMusica.Text+'?'),'Atenção!',MB_YESNO+MB_ICONWARNING) = ID_YES) then
             begin
         ...
         begin
            if (Application.MessageBox(pchar('Deseja Excluir a Música '+cmbNomeMusica.Text+'?'),'Atenção!',MB_YESNO+MB_ICONWARNING) = ID_YES) then
             begin
    ...
    Except on E: Exception do
     begin
        Application.MessageBox(pchar('ERROR: '+E.Message),'Cadastro Execução', MB_OK);
        FreeAndNil(ECAD);
        CloseModal;
     end;
