...
type
  TfrmConsultaMusicaAutor = class(TForm)
    Panel1: TPanel;
...
var
  frmConsultaMusicaAutor: TfrmConsultaMusicaAutor;

implementation
uses dtmECAD;
{$R *.dfm}

procedure TfrmConsultaMusicaAutor.btnPesquisarClick(Sender: TObject);
begin
    if cmbConsulta.Text = '' then
    begin
       Application.MessageBox('Escolha um Autor','Atenção',MB_OK+MB_ICONEXCLAMATION);
...
procedure TfrmConsultaMusicaAutor.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  FreeAndNil(ECAD);
end;

procedure TfrmConsultaMusicaAutor.FormCreate(Sender: TObject);
begin
