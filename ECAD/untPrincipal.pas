...
type
  TfrmPrincipal = class(TForm)
    ...
    MesVerba: TMenuItem;
    MesVerbaAutor: TMenuItem;
    AutorMesVerba: TMenuItem;
...
uses untCadastroAutores, untCadastroMusica, untCadastroExecMusica, untProcessaPagamento,
     ...
     untConsultaMesVerba,untConsultaMesAutorVerba,untConsultaAutorMesVerba;
{$R *.dfm}
...
begin
  ...
  else if (TMenuItem(sender).name = 'MesVerba') then
    begin
  ...
  else if (TMenuItem(sender).name = 'MesVerbaAutor') then
    begin
        Application.CreateForm(TfrmConsultaMesAutorVerba,frmConsultaMesAutorVerba);

        try
            frmConsultaMesAutorVerba.ShowModal ;
        finally
            FreeAndNil(frmConsultaMesAutorVerba);
        end;
    end
  else if (TMenuItem(sender).name = 'AutorMesVerba') then
    begin
        Application.CreateForm(TfrmConsultaAutorMesVerba,frmConsultaAutorMesVerba);

        try
            frmConsultaAutorMesVerba.ShowModal ;
        finally
            FreeAndNil(frmConsultaAutorMesVerba);
        end;
    end;
